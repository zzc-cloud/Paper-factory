...
1. **准备素材** — 在 `workspace/{project}/input-context.md` 描述项目背景和创新点（或使用 `codebase-analyzer` 从代码库自动生成）
...
**并行执行优化**多Phase 1 支持真正的并行执行模式（通过 `config.json` 配置），预期能减少 40-60% 的执行时间。详见 [phase1-parallel-optimization](docs/phase1-parallel-optimization.md)。

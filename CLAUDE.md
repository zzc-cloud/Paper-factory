...
**并行执行优化**：Phase 1 支持真正的并行执行模式（通过 `config.json` 配置），预期能减少 40-60% 的执行时间。详见 [phase1-parallel-optimization](docs/phase1-parallel-optimization.md)。
...
│       └── d1-reviewer-domain-expert.md  # 领域评审专家模板
├── docs/                  # 完整文档
...
| [技能目录](docs/skills-catalog.md) | 已安装技能的分类索引（含 7 个领域评审认知框架） |
...
1. **venue-analyzer** — 期刊配置解析器
   - 解析 `venues.md` 中���期刊配置（包括写作风格、审稿标准、历史数据）
...
3. **feedback-collector** — 反馈收集器
   - 收集用户在各交互节点的反馈
   - 结构化存储到 `workspace/{project}/user-feedback.json`
   - 自动应用用户反馈并调整后续策略

4. **Phase 0** — 交互式启动确认阶段
   - 在正式进入文献调研（Phase 1）之前的交互式确认
   - 确保论文方向正确，减少 60-70% 的后期返工成本

### 更新的 Skill

- `.claude/skills/venue-analyzer/` — 期刊配置解析器
- `.claude/skills/interaction-manager/` — 交互管理器
- `.claude/skills/feedback-collector/` — 反馈收集器

### 更新的配置

- `config.json` 新增：
  - `interaction` 配置：交互模式、关键检查点、自动应用反馈
  - `venue_analysis` 配置：期刊分析开关、预定义期刊列表
...
- **环节化确认与调整**：在生成论文题目、摘要等关键环节让用户进行确认和调整
- **期刊属性定位**：根据用户投稿的具体期刊，动态适配格式、风格和内容要求

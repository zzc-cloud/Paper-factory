...
Data Analysis and Machine Learning 研究基于以下核心范式：

1. **数据驱动范式**：从数据中发现模式，而非依赖先验假设
2. **统计推断范式**：从样本推断总体，量化不确定性
3. **预测建模范式**：构建模型预测未来或未知数据
4. **可重复性范式**：确保分析可重复、结果可靠
...
**深度学习**：ResNet, Transformer, BERT, GNN

**领域方法**：根据具体应用领域

**基准数据集**：UCI, Kaggle, ImageNet, 等

### 领域特定评审问题

1. **特征选择**：特征选择是否有理论或实证支持？

2. **消融实验**：是否进行了充分的消融实验验证各组件贡献？

3. **类别不平衡**：如何处理类别不平衡问题？

4. **统计显著性**：是否提供了统计显著性检验和置信区间？

5. **可重复性**：实验是否可重复（代码、数据公开）？

6. **基线对比**：与基线方法的比较是否公平（相同设置、数据）？

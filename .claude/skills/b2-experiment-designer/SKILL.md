---
name: b2-experiment-designer
description: "实验设计师 — 设计严格的实验方案验证创新点，定义评估指标和基线。"
---
...
`Skill(skill="b2-experiment-designer", args="{project}")`
...
3. `workspace/{project}/phase1/` 目录包含 Phase 1 输出文件
...
**智能体输出**（由条件激活的智能体产生）：
- `a1-literature-survey.json`——文献调研和研究全景（始终存在）
- `b1-related-work.json`——相关工作分析（始终存在）
...
- `data.research_landscape`、`data.gaps`（在 A1 中）
- `data.positioning`、`data.comparison_matrix`（在 B1 中）
...
- 如果仅有 `input-context.md`、`a1-literature-survey.json` 和 `innovation-synthesis.json` 存在：请根据 `input-context.md` 中的创新声称和系统描述设计实验。注明架构细节有限。
- 如果 Skill 输出存在：请使用理论声明来设计理论验证协议。
...
2. 阅读 `innovation-synthesis.json` 获取聚合创新声明
3. 使用 Glob 发现所有可用的分析文件：`workspace/{project}/phase1/a*.json`、`workspace/{project}/phase1/skill-*.json` 和 `workspace/{project}/phase2/b1-related-work.json`
4. 阅读每个发现的文件并提取相关证据：
   - 来自 A1（如果可用）：研究全景、缺口分析
   - 来自 B1（如果可用）：相关工作定位、竞争对手分析
   - 来自技能输出（如果可用）：理论声明、领域特定分析、形式属性
5. 创建内部映射：创新声明 -> 所需实验证据
### 步骤 2：定义评估指标

设计直接验证创新聚合中创新声明的指标。每个指标必须有精确的数学定义。请遵循此方法论：

1. **将创新映射到可测量结果**：对于创新聚合中的每个创新声明，识别什么可观察量将确认或反驳它。
...
为对照比较设计 3-6 个基线系统。从系统的架构和创新声明（创新聚合）派生基线。每个基线应隔离一个特定的架构决策或组件。

请遵循此基线设计方法论：

**基线类型 1：朴素/无结构基线**
- 描述：完成相同任务的最简单合理方法，没有系统的关键结构创新
- 测试内容：系统的核心结构贡献（例如多本体、知识图谱、专门架构）是否比更简单的方法提供价值
...
设计隔离每个关键架构组件的消融研究。直接从创新聚合文件派生消融列表——每个主要创新声明应有相应的消融来移除或禁用该组件。
...
5. 覆盖创新聚合中的所有主要创新声明——如果某个创新无法消融，请说明原因
...
> **指导**：具体的类别、示例和基本真字段完全取决于系统的任务。请阅读 `input-context.md` 以了解系统产生什么，然后设计在不同难度级别上练习其组件的类别。
### 步骤 6：设计实验协议
...
**协议 3：理论声明验证**
- 步骤：
  1. 对于来自 Skill 输出和创新聚合的每个理论声明，设计测量程序
...
如果系统具有理论贡献（来自 Skill 输出和创新聚合），为每个理论设计严谨的测量方法论。

对于每个理论声明：

1. **形式陈述**：用精确术语重述来自 Skill 输出和创新聚合的理论声明
...
```json
{
  ...
  "data": {
    ...
    "theoretical_validation": {
      "claims": [
        {
          "claim": "Theoretical claim from Skill outputs or innovation synthesis",
...
- **组件细节**：详细架构、数据流和组件交互（如由 codebase-analyzer 生成）

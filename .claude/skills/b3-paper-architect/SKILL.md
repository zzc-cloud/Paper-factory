---
name: b3-paper-architect
description: "论文架构师 — 设计完整的论文结构、章节划分和内容分配。"
---
...
`Skill(skill="b3-paper-architect", args="{project}")`
...
2. `workspace/{project}/phase2/b2-experiment-design.json` 存在
...
1. 阅读所有 Phase 1 输出（A1、B1、创新聚合）和 Phase 2 输出（B2）
...
2. `workspace/{project}/phase1/innovation-synthesis.json` —— 始终存在（Phase 1 聚合）
...
- 从 input-context.md：系统是什么？什么领域？声称了什么创新？架构细节和设计决策是什么？
...
**关键洞察**（"顿悟"时刻是什么？）：
- 从创新聚合的顶级创新声明派生多核心概念突破是什么？
- 此方法与先前工作根本不同在哪里？
- 表达为一个单一、可记忆的想法，论文围绕它构建

**解决方案**（系统如何工作？）：
- 从 input-context.md 派生：系统的高级架构是什么？
- 关键组件是什么，它们如何交互？
- 架构如何体现关键洞察？
**验证**（读者为什么应该相信它有效？）：
- 从 B2 派生：计划了什么实验？
...
3. 关键洞察（1 句话）：核心概念突破（来自创新聚合）
4. 解决方案概览（2-3 句话）：高级架构和关键机制（来自 input-context.md）
...
- 3.1 概览
  - 关键论点：来自 input-context.md 的高级架构
  - 图表：系统架构图
- 3.2+ 组件细节
...
> **第 3 节指导**：子节数量和内容完全取决于系统的架构（input-context.md）。具有 3 层的系统与具有 5 个模块的系统需要不同的子节。请仔细阅读 input-context.md 并设计与实际架构匹配的子节。
...
指定每个图表，包含足够的细节供可视化设计师使用。根据系统的架构、理论声明（来自 Skill 输出）和实验设计（B2）派生图表内容。
...
指定每个表格，包含列、用途和位置。根据系统的架构（input-context.md）、相关工作（B1）和实验设计（B2）派生表格内容。
...
5. 为什么设计选择重要：为关键架构决策（来自 input-context.md/创新聚合）辩护
...
```json
{
  ...
  "data": {
    ...
    "narrative_arc": {
      ...
      "insight": "关键洞察派生自创新聚合的顶级创新声明（中文描述）",
...
- **系统名称**：用于标题、摘要和整个大纲
- **领域**：决定问题框架和会场选择
- **架构概览**：驱动第 3 节结构（包含组件、数据流等详细信息）
- **创新列表**：驱动贡献列表、章节分配和叙事弧
- **关键度量/规模**：为问题陈述和系统描述提供具体数字
- **提议标题**（如果存在）：步骤 3 的起点

### 从 Phase 1 输出（A1、创新聚合、Skill 输出）：
- **A1**：研究全景、被引用论文、缺口（驱动第 2 节和问题框架）
- **创新聚合**：聚合创新及新颖性论证（驱动贡献和叙事）
- **Skill 输出**：理论属性、形式声明（驱动第 4 节）

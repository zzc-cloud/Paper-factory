...
Knowledge Graph (KG) 研究基于以下核心范式：

1. **语义表示范式**：知识以三元组（subject, predicate, object）形式建模，支持推理和查询
2. **本体工程范式**：通过形式化本体（OWL/RDFS）定义 schema，支持约束验证
3. **图结构范式**：利用图的拓扑特性（路径、社区、中心性）进行知识发现
4. **神经符号融合范式**：结合符号推理的准确性和神经网络的泛化能力
...
**SOTA 对比清单**：
- 图神经网络：RGCN, CompGCN, HGT
- KGE 方法：TransE, RotatE, ConvE, TuckER
- 预训练模型：KEPLER, K-BERT
...
**KGE 方法**：TransE, DistMult, ComplEx, RotatE, ConvE, TuckER, HAKE

**图神经网络**：RGCN, CompGCN, HGT, GraphBERT

**KG 完成任务**：链接预测 (MRR, Hits@1/3/10)、三元组分类

**推理任务**：本体推理 (分类、实例检查)、规则推理

### 领域特定评审问题

1. **本体表达能力**：论文声称的本体表达能力是否与使用的 OWL 子语言匹配？

2. **图数据规模**：图数据规模是否充分支撑实验结论（节点数、边数、属性数）？

3. **本体对齐/融合**：是否讨论了本体对齐/融合的挑战？

4. **推理方法对比**：推理方法是否与传统推理机（如 Pellet、HermiT）对比？

5. **KG 嵌入评估**：KG 嵌入方法是否与最新的图神经网络方法对比？

6. **可扩展性**：系统的可扩展性如何评估（数据规模增加时的性能）？

7. **推理复杂度**：推理复杂度是否与声称的能力一致？

8. **标准遵循**：是否遵循 W3C 标准（RDF, OWL, SPARQL）？

9. **多模态支持**：是否考虑了多模态知识（文本、图像、数值）？

10. **知识更新**：是否讨论了知识更新和增量学习？

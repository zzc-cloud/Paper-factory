...
>
> **V2 新增**：交互式论文生成流程 — 读取扩展的期刊配置（包括写作风格、审稿标准、历史数据），生成 `venue-style-guide.md` 指导后续 Phase 的写作风格适配。

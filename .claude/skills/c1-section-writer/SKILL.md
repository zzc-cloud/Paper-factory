---
name: c1-section-writer
description: "章节撰写专家 — 根据论文大纲逐章节撰写学术内容。"
---
...
`Skill(skill="c1-section-writer", args="{project}:{section_id}:{section_name}")`

## 参数解析

从 `args` 解析（以 `:` 分隔）：
- `project` — 项目目录名称（必需）
- `section_id` — 章节编号，如 "01", "02"（必需）
- `section_name` — 章节名称，如 "introduction", "related-work"（必需）

...
**调用方式：** 这些函数通常由 `paper-phase1-research` Skill 调用，也可独立使用。
...
**功能：** 解析论文 Markdown 文件的 Frontmatter
...
| `source` | string | ✅ | 来源："websearch" \| "manual" |
| `added_date` | string | ✅ | 添加到缓存的日期 (ISO 8601) |
| `tags` | array | ✅ | 标签/关键词 |
| `status` | string | ✅ | 阅读状态："read" \| "skm" \| "todo" |
...
用户可以在 `workspace/{project}/.cache/papers/{domain}/` 下直接创建 `.md` 文件：
...
- **检测**：`!Glob("workspace/{project}/.cache/papers/{domain}/*")`
- **处理**：自动调用 `initCache(project, domain)`
- **日志**：记录"缓存目录不存在，已初始化"

### Frontmatter 解析失败

- **检测**：`!markdownContent.match(/^---$/m)`
- **处理**：跳过该文件，记录警告
- **日志**：`"Warning: {filename} has invalid Frontmatter, skipping"`

### ID 冲突

- **检测**：尝试写缓存时发现 `id` 已存在
- **处理**：覆盖旧文件（用户可能更新了论文信息）
- **日志**：`"Info: Updated existing paper {id}"`

---

## 配置支持

缓存系统支持从 `config.json` 读取配置：

...
Natural Language to SQL (NL2SQL/Text2SQL) 研究基于以下核心范式：

1. **语义映射范式**：将自然语言映射到结构化查询语言（SQL）
2. **Schema Linking 范式**：建立自然语言短语与数据库 schema 的链接
3. **神经符号范式**：结合神经网络的语言理解能力和符号化的 SQL 语法
4. **交互式范式**：通过多轮交互澄清查询意图
...
**SOTA 对比清单**：
- 传统方法：Seq2SQL, TypeSQL, IRNet
- LLM 方法：GPT-4 based, DAIL-SQL, C3
- 基准：Spider, WikiSQL, CoSQL, SParC
...
**传统神经方法**：Seq2SQL, TypeSQL, IRNet, X-SQL, RAT-SQL

**LLM 方法**：DAIL-SQL, C3, GPT-4 based, CoderX

**基准数据集**：Spider, WikiSQL, CoSQL, SParC, KaggleDBQA

**评估指标**：Exact Match, Execution Accuracy, Efficiency

### 领域特定评审问题

1. **Schema Linking 歧义处理**：Schema Linking 模块如何处理歧义（如同名列）？

2. **复杂查询支持**：是否支持多跳查询和复杂嵌套？

3. **泛化能力**：如何处理超出训练 schema 的查询？

4. **基准对比**：是否与 Spider、WikiSQL 等基准对比？

5. **跨数据库泛化**：是否评估了跨数据库泛化能力？

6. **用户交互**：是否支持多轮交互澄清？

7. **错误恢复**：查询失败时如何反馈和恢复？

8. **LLM 对比**：是否与最新的 LLM-based 方法对比？

9. **查询优化**：是否考虑了查询执行效率？

10. **真实部署**：是否有真实场景的部署和评估？

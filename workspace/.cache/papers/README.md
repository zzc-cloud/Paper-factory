...
本目录用于存储论文工工项目检索到的论文缓存，避免重复检索并支持用户手动添加论文。
...
1. **ID 唯一性**多每篇论文的 `id` 必须唯一，推荐使用 `{source}-{year}-{标识}` 格式
2. **source 字段**多使用 `websearch` 或 `manual`，系统据此区分来源
3. **status 更新**多可以随时更新论文的阅读状态（todo → skim → read）
4. **目录组织**多不要在不同领域间移动论文文件，系统通过目录结构识别领域

...
学术论文写作是一项复杂的智力工程，需要多种截然不同的专业能力多

- **文献调研**多需要广泛检索、快速筛选、精准分类数十篇相关论文
- **工程分析**多需要深入理解代码架构、设计模式、技术创新点
- **理论建模**多需要将工程实践抽象为学术贡献，建立形式化框架
- **实验设计**多需要设计对比实验、选择评价指标、预测结果
- **学术写作**多需要遵循严格的论证逻辑、符合顶会规范、语言精炼准确
- **同行评审**多需要从多个维度批判性审视论文质量

传统的单人写作模式下，研究者需要在这些角色间频繁切换，认知负担极高。即使是单个 AI 助手，也难以在一次对话中同时保持"广度检索"与"深度分析"、"批判性思维"与"建设性写作"的平衡。

Paper Factory 的核心洞察是多**学术论文生成本质上是一个多角色协作的工作流，而非单一智能体的独角戏**。
...
**理念**多让每个 agent 只做一件事，并把它做到极致。

在真实的学术团队中，文献调研员、系统架构师、理论学家、实验设计师、论文写手、审稿人是不同的角色。Paper Factory 将这种分工映射到 11 个专业 agent多

- **A1 (Literature Surveyor)**多专注于文献检索，使用 Sonnet 模型快速处理大量论文元数据
- **A3 (MAS Theorist)**多研究多智能体系统理论，需要 Opus 的抽象能力
- **C1 (Section Writer)**多流畅撰写章节内容，Sonnet 的语言生成能力足够
- **D1 (Peer Reviewer)**多模拟三位独立审稿人，需要 Opus 的批判性思维

**技术实现**多
- 每个 agent 的系统提示（`agents/{phase}/{agent}.md`）精确定义其角色、输入、输出、评价标准
- 不同任务匹配不同模型多Opus 用于深度推理（$4-5 预算），Sonnet 用于高吞吐任务（$2-3 预算）
- 工具权限隔离多写作 agent 只能写文件，评审 agent 只能读文件
...
**理念**多学术论文写作有天然的依赖顺序，不能"边调研边写作"。

Paper Factory 将流程分解为 4 个阶段多
...
- Phase 内部的依赖通过 agent 生成顺序控制（如 A1/A3 完成后才生成 A4）
...
**理念**多一次写作很难完美，需要模拟真实的同行评审流程。

Phase 4 实现了一个闭环优化机制多
...
**为什么是 3 次迭代**多
- 第 1 次多修正明显错误（逻辑漏洞、实验缺失）
- 第 2 次多优化表达（语言精炼、结构调整）
- 第 3 次多细节打磨（术语统一、引用完善）
- 超过 3 次通常收益递减
...
Paper Factory 的创新在于多**整个 pipeline 由 `CLAUDE.md` 文件驱动，Team Lead 读取后自主执行**。

**CLAUDE.md 的内容**多
- Agent 定义表（11 个 agent 的角色、模型、预算）
...
**为什么这样设计**多
- **无需外部依赖**多Claude Code + Agent Teams 原生支持
- **人类可理解**多CLAUDE.md 是自然语言，不是代码
- **易于调试**多修改 CLAUDE.md 即可调整流程，无需重新部署
- **版本控制**多CLAUDE.md 随项目一起提交到 Git
...
**定位**多Team Lead 是指挥家，不是演奏家。

在 Paper Factory 中，Team Lead 的职责是多
- **编排**多决定何时生成哪个 teammate
- **协调**多管理 agent 间的依赖关系（如 A4 依赖 A1/A3）
- **质量把控**多执行 Quality Gate，决定是否进入下一阶段
- **异常处理**多当 agent 失败时，决定重试还是通知用户

Team Lead **不做**的事情多
- 不直接写论文内容（委派给 C1）
- 不直接分析代码（委派给 codebase-analyzer 前置工具）
- 不直接评审论文（委派给 D1）

**为什么这样设计**多
- **专业化**多Team Lead 专注于流程管理，写作质量由专业 agent 保证
- **可扩展**多增加新 agent 只需修改 CLAUDE.md，Team Lead 逻辑不变
- **避免上下文污染**多Team Lead 的对话历史不会混入大量论文内容

### 文件系统通信

**理念**多Agent 间通过文件交换信息，而非对话历史。

每个 agent 的输入输出都是明确的文件路径多
...
**为什么不用消息传递**多
- **持久化**多文件可以被人类检查、修改、备份
- **解耦**多Agent 不需要知道谁生成了输入文件
- **可恢复**多如果某个 agent 失败，可以手动补充文件后继续
- **并行友好**多多个 agent 可以同时读取同一个文件

**文件格式约定**多
- **JSON**多结构化数据（如文献列表、实验设计、评审分数）
- **Markdown**多人类可读的报告（如工程分析报告、论文章节）
...
Claude Code 的 Agent Teams 提供了以下关键能力多

1. **并行执行**多Phase 1 中 A1/A3 可以同时运行，节省时间
2. **消息传递**多Agent 间可以通过 `SendMessage` 工具通信（虽然 Paper Factory 主要用文件）
3. **预算控制**多每个 agent 可以设置独立的预算上限
4. **原生集成**多无需外部 API 调用或进程管理

**对比其他方案**多
- **LangGraph**多需要 Python 代码定义图结构，不如 CLAUDE.md 直观
- **AutoGen**多需要编写 agent 类，部署复杂
- **手动调用 API**多需要自己实现并行、重试、状态管理

### 为什么用 CLAUDE.md 驱动

**单一事实来源（Single Source of Truth）**多

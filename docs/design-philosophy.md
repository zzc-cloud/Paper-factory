...
在真实的学术团队中，文献调研员、系统架构师、理论学家、实验设计师、论文写手、审稿人是不同的角色。Paper Factory 将这种分工映射到 10 个专业 agent：

- **A1 (Literature Surveyor)**：专注于文献检索和分类，使用 Opus 模型确保检索质量和相关性判断
- **B1 (Related Work Analyst)**：系统性文献比较和缺口分析，需要 Opus 的深度推理
- **C1 (Section Writer)**：流畅撰写章节内容，Sonnet 的语言生成能力足够
- **D1 (Peer Reviewer)**：模拟三位独立审稿人，需要 Opus 的批判性思维

**技术实现**：
- 每个 agent 的系统提示（`.claude/skills/{agent-id}/SKILL.md`）精确定义其角色、输入、输出、评价标准
...
- Phase 内部的依赖通过 agent 生成顺序控制（如 B1 依赖 A1 的输出）
...
```json
{
  ...
  "files_expected": [
    ...
    "phase1/innovation-synthesis.json",
    "phase1/innovation-synthesis.md"
  ],
  "files_found": [...],
  "files_missing": []
}
...
for iteration in 1..15:
  1. D1 (Peer Reviewer) 评审 paper.md
     - 协调多视角评审专家（Technical、Domain、Clarity、Writing Quality 等）
     - 从 Originality、Significance、Methodology、Clarity、Writing Quality、Technical Soundness 六个维度打分
     - 输出 average_score 和详细意见

  2. 检查 average_score
     - 若 >= 9.0 → 通过，结束循环
     - 若 < 9.0 且 iteration < 15 → 进入修订
...
**为什么最多 15 次迭代**：
- 前 1-3 次：修正明显错误（逻辑漏洞、实验缺失）
- 第 4-8 次：优化表达（语言精炼、结构调整）
- 第 9-15 次：细节打磨（术语统一、引用完善）
- 配合 9.0 的高阈值，确保论文达到顶会水准
- 实际大多数论文在 5-8 次迭代内达标
...
**CLAUDE.md 的内容**：
- Agent 定义表（10 个 agent 的角色、模型、预算）
...
4. 按阶段执行：
   - 生成 teammate（通过 Skill 调用 `.claude/skills/{agent-id}/SKILL.md`）
   - 等待 teammate 完成
   - 执行 Quality Gate
   - 决定下一步行动（继续/重试/通知用户）
```
...
- **协调**：管理 agent 间的依赖关系（如 B1 依赖 A1 的输出）
...
- B1 读取 `phase1/a1-literature-survey.json` 和 `phase1/innovation-synthesis.json`
...
1. **串行执行**：Phase 1 中 A1 → B1 串行执行，确保依赖关系

...
| 层级 | 组件 | 作用 | 通信模式 |
|--------|------|----------|----------|
| **Orchestrator Level** | `paper-generation` | 总编排器，管理完整 4 阶段流程 | Skill 同步调用 |
| **Phase Level** | 4 个 Phase Skill | 阶段编排，串行管理流程 | Skill 同步调用 |
| **Agent 层** | 10 个 Agent 组件 | 专业化任务执行 | 文件系统通信 |
| **辅助 Skill 层** | 领域知识、缓存、版本管理等 | 数据管理与工具支持 | Skill 同步调用 |
...
| **Phase 编排** |
| `paper-phase1-research` | Phase 1 编排器，串行执行模式 |

...
本文档列出 Paper Factory 系统中所有 12 个 Agent 的完整定义、职责和配置信息。
...
| Phase 4 | D1-Technical-Expert | ��术评审专家 | Opus | $5 |
...
**Prompt 文件**：`agents/phase1/a1-literature-surveyor.md`
...
### A2 - Engineering Analyst（工程分析专家）

**Prompt 文件**：`agents/phase1/a2-engineering-analyst.md`

**核心职责**：
- 深度分析目标代码库的架构、设计模式、技术栈
- 提取工程创新点和关键技术决策
- 识别系统的可扩展性和设计权衡

**输入**：
- `workspace/{project}/input-context.md`
- 目标代码库路径（来自 `codebase_path` 字段）

**输出**：
- `workspace/{project}/phase1/a2-engineering-analysis.json`
- `workspace/{project}/phase1/a2-engineering-analysis.md`

**工具**：Read, Glob, Grep, Write, Bash

**激活**：条件（当 `input-context.md` 中包含有效的 `codebase_path` 时）

---

### A3 - MAS Literature Researcher（MAS 文献调研）

**Prompt 文件**：`agents/phase1/a3-mas-theorist.md`
...
**激活**：条件（当项目深度涉及多智能体架构且需要最新文献支撑时）

---

### A4 - Innovation Formalizer（创新形式化专家）

**Prompt 文件**：`agents/phase1/a4-innovation-formalizer.md`
...
**输入**：
- `workspace/{project}/phase1/a2-engineering-analysis.json`（依赖）
- `workspace/{project}/input-context.md`
...
**Prompt 文件**：`agents/phase2/b1-related-work-analyst.md`
...
**Prompt 文件**：`agents/phase2/b2-experiment-designer.md`
...
**Prompt 文件**：`agents/phase2/b3-paper-architect.md`
...
**Prompt 文件**：`agents/phase3/c1-section-writer.md`
...
- 例如：`01-introduction.md`, `02-related-work.md`, `03-methodology.md` ...
...
**Prompt 文件**：`agents/phase3/c2-visualization-designer.md`
...
**Prompt 文件**：`agents/phase3/c3-academic-formatter.md`
...
**输入**：
- 所有章节文件：`workspace/{project}/phase3/sections/*.md`
- 图表文件：`workspace/{project}/phase3/figures/*.md`
- 文献数据：`workspace/{project}/phase1/a1-literature-survey.json`
...
- **其他专家**（固定启用）：Technical、Clarity、Writing 始终启用，提供基础评审维度
- **领域知识准备**：选中的领域专家先通过 `domain-knowledge-prep` Skill 准备知识
- **资源优化**：不启动低相关度的领域专家，避免无关专家给出低质量反馈
...
**模式 B：单一综合评审**
- **原 D1 Agent**：`agents/phase4/d1-peer-reviewer.md`（已保留作为参考）

**Prompt 文件**：`agents/phase4/d1-peer-reviewer.md`
...
**Prompt 文件**：`agents/phase4/d2-revision-specialist.md`

**核心职责**：
- 根据评审意见修订论文
- 保持学术语气和论证连贯性
- 记录所有修改内容
- **专家辩论协调**：向 5 个评审专家发送个性化回复并处理反馈
...
- `a2-engineering-analysis.json` + `.md`
...
|-------|----------|------|
| A1 | 必选 | 文献调研是所有论文的基础 |
| A2 | 有 codebase_path | 仅在有代码库分析时启动 |
...
添加新 Agent 时，需要同步更新以下文件：

1. **创建 Prompt 文件**：在 `agents/{phase}/` 下创建 `{agent-id}-{name}.md`
2. **更新配置**：在 `config.json` 中添加模型和预算配置
3. **更新本目录**：在上述表格中添加新 Agent 的定义
4. **更新架构图**：在 `docs/architecture.md` 中更新数据流图
5. **更新 Quality Gate**：在对应 Phase 的 Gate 检查中添加新输出文件
